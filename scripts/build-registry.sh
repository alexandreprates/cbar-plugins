...
# system
add_plugin "system.cpu-chart" "CPU Chart" "system" "Shows CPU usage history as a compact panel chart." "1.1.0" "plugins/system/cpu-chart.5s.sh" "5s" "bash" "en" "awk,base64,mkdir,tr" "CBAR_CPU_WARN" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
add_plugin "system.memory-gauge" "Memory Gauge" "system" "Shows RAM usage as a compact panel gauge." "1.1.0" "plugins/system/memory-gauge.5s.sh" "5s" "bash" "en" "awk,base64,tr" "CBAR_MEMORY_WARN,CBAR_MEMORY_CRIT" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
add_plugin "system.disk" "Disk" "system" "Shows filesystem usage as compact segmented blocks." "1.1.0" "plugins/system/disk.30s.sh" "30s" "bash" "en" "awk,base64,df,tr" "CBAR_DISK_PATH,CBAR_DISK_WARN,CBAR_DISK_CRIT" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
add_plugin "system.os-health" "OS Health" "system" "Shows CPU usage, available memory, and available disk as three compact health bars." "1.0.0" "plugins/system/os_health.30s.sh" "30s" "bash" "en" "awk,base64,df,python3,sleep,tr" "CBAR_OS_HEALTH_DISK_PATH,CBAR_OS_HEALTH_CPU_WARN,CBAR_OS_HEALTH_CPU_CRIT,CBAR_OS_HEALTH_MEMORY_WARN,CBAR_OS_HEALTH_MEMORY_CRIT,CBAR_OS_HEALTH_DISK_WARN,CBAR_OS_HEALTH_DISK_CRIT" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
...
add_plugin "network.vpn-status" "VPN Status" "network" "Shows VPN connection status with interface and copy actions." "1.1.0" "plugins/network/vpn-status.10s.sh" "10s" "bash" "en" "awk,base64,cat,ip,nmcli,tr,wl-copy,xclip" "CBAR_VPN_INTERFACE" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
add_plugin "network.ssh-hosts" "SSH Hosts" "network" "Shows configured SSH host reachability with compact status." "1.2.0" "plugins/network/ssh-hosts.30s.sh" "30s" "bash" "en" "awk,base64,date,ssh,tr,wl-copy,xclip" "CBAR_SSH_HOSTS,CBAR_SSH_WARN_MS,CBAR_SSH_TIMEOUT" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
add_plugin "network.ping" "Ping" "network" "Shows compact latency to a configurable host." "1.1.0" "plugins/network/ping.10s.sh" "10s" "bash" "en" "awk,ping" "CBAR_PING_HOST,CBAR_PING_WARN_MS,CBAR_PING_CRIT_MS" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
...
add_plugin "dev.kubernetes-context" "Kubernetes Context" "dev" "Shows current Kubernetes context and namespace with production alerts." "1.1.0" "plugins/dev/kubernetes-context.30s.sh" "30s" "bash" "en" "base64,kubectl,python3,tr,wl-copy,xclip" "CBAR_KUBE_PROD_PATTERN" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
add_plugin "dev.openai-codex" "OpenAI Codex Usage" "dev" "Displays OpenAI Codex usage limits from local Codex session metadata." "1.1.0" "plugins/dev/openai_codex.5m.sh" "5m" "bash" "en" "python3,sed,tr" "CBAR_CODEX_SHOW_7D,CBAR_CODEX_COLORS,CBAR_CODEX_SHOW_RESET,CBAR_CODEX_SHOW_BARS" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"

# productivity
add_plugin "productivity.keep-awake" "Keep Awake" "productivity" "Keeps the current desktop session awake with a toggleable inhibitor." "1.1.0" "plugins/productivity/keep-awake.1m.sh" "1m" "bash" "en" "base64,cat,chmod,date,grep,mkdir,pgrep,python3,rm,systemd-inhibit,tr" "CBAR_AWAKE_WHAT,CBAR_AWAKE_REASON" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
add_plugin "productivity.timer" "Timer" "productivity" "Shows a compact animated hourglass countdown timer." "1.1.0" "plugins/productivity/timer.1s.sh" "1s" "bash" "en" "base64,date,mkdir,rm,tr" "CBAR_TIMER_SECONDS" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
