...

{
  ...
  for plugin in "${plugins[@]}"; do
    IFS='|' read -r id name category description path interval language dependencies env license publisher publisher_url <<< "${plugin}"
    ...
    if [[ -z "${publisher}" ]]; then
      printf 'missing publisher metadata: %s\n' "${id}" >&2
      exit 1
    fi
