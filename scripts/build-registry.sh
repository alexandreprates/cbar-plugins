...
add_plugin "system.service-status" "Service Status" "system" "Shows systemd service health with compact status and actions." "plugins/system/service-status.30s.sh" "30s" "bash" "en" "awk,base64,journalctl,systemctl,tr" "CBAR_SERVICE_UNITS" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
add_plugin "system.updates-available" "Updates Available" "system" "Shows available APT and Flatpak updates with a compact badge." "plugins/system/updates-available.30m.sh" "30m" "bash" "en" "apt,awk,base64,cosmic-store,flatpak,tr" "CBAR_UPDATES_INCLUDE_APT,CBAR_UPDATES_INCLUDE_FLATPAK" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
...
add_plugin "network.ssh-hosts" "SSH Hosts" "network" "Shows configured SSH host reachability with compact status." "plugins/network/ssh-hosts.30s.sh" "30s" "bash" "en" "awk,base64,date,ssh,tr,wl-copy,xclip" "CBAR_SSH_HOSTS,CBAR_SSH_WARN_MS,CBAR_SSH_TIMEOUT" "GPL-3.0-only" "AlexandrePrates" "https://github.com/AlexandrePrates"
